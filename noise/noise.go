...
// NoiseType indicates which type of noise MakeNoise will generate
type NoiseType int
...
// MakeNoise generates 2d block of noise function values
func MakeNoise(noiseType NoiseType, frequency, lacunarity, gain float32, octaves int, w, h int) (noise []float32, min, max float32) {
	noise = make([]float32, w*h)
	min = float32(999999.0)
	max = float32(-999999.0)
	var mutex = &sync.Mutex{}

	numRoutines := runtime.NumCPU()
	var wg sync.WaitGroup
	wg.Add(numRoutines) // we are telling the wait group to wait this many times

	batchSize := len(noise) / numRoutines
	for i := 0; i < numRoutines; i++ {
		go func(i int) {
			defer wg.Done() // let wg know we are done here
			start := i * batchSize
			...
			for j := start; j < end; j++ {
				...
				if noiseType == TURBULENCE {
					...
					noise[j] = fractal(float32(x), float32(y), frequency, lacunarity, gain, octaves)
				}

				if noise[j] > max || noise[j] < min {
					mutex.Lock()
					if noise[j] > max {
						max = noise[j]
					} else if noise[j] < min {
...
// lacunarity - rate at which we will be changing the frequency with each iteration
// gain - rate at which we will be chaning the amplitude in each iteration
// octaves - how many iterations we are going to do
// 3 octaves means we will be generating noise three times and kind of composing the results
func fractal(x, y, frequency, lacunarity, gain float32, octaves int) float32 {
	var sum float32
